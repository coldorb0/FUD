...
Everything is now generated in two files, fireworm.txt and unicorn.rc. The text file contains all
...
def format_payload(powershell_code, attack_type, attack_modifier, option):
    ...
    if attack_type == "msf":
        if attack_modifier == "macro":
            ...
            write_file("fireworm.txt", macro_attack)
            macro_help()
            ...
            write_file("fireworm.txt", full_attack)
            ps_help()
    ...
    if attack_type == "msf" and attack_modifier == "hta":
        ...
        print("[*] Exported powershell output code to fireworm.txt.")
